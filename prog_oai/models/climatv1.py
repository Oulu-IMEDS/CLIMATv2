...
class CLIMATv1(nn.Module):
    def __init__(self, cfg, device, pr_weights=None, pn_weights=None, y0_weights=None):
        super().__init__()
        # 'Img'
        # 'Age', 'BMI',	'KL',
        ...
        if "DT" in self.input_data:
            self.dt_ft = self.create_metadata_layers(cfg.seq_len, self.n_meta_features)
        ...
        self.feat_fusion = FeaT(num_patches=self.n_metadata, with_cls=True, num_cls_num=1,
                                patch_dim=self.n_meta_features, gradings=(), use_separate_ffn=True,
        ...
        self.feat_prognosis = FeaT(num_patches=self.n_patches + 1, with_cls=True, num_cls_num=self.num_cls_num,
                                   patch_dim=self.feat_dim, gradings=self.gradings, use_separate_ffn=True,
                                   ...
                                   emb_dropout=cfg.feat_emb_drop_rate, n_outputs_per_grading=cfg.seq_len)

        self.output_map_r = self.feat_diag.output_map
        self.output_map_p = self.feat_prognosis.output_map
    ...
    def forward(self, input, batch_i=None, target=None):
        ...
        preds_r, preds_p, d_attn, f_attn, p_attn = self.apply_feat(img_features, meta_features)
        ...
        if self.cfg.save_attn:
            self.save_attentions(batch_i, root=self.cfg.log_dir, img=input['IMG'] if "IMG" in self.input_data else None,
                                 metadata_names=self.input_data, diags=preds_r,
                                 d_attn=d_attn, f_attn=f_attn, p_attn=p_attn, preds=preds_p, targets=target)
    ...
    def apply_feat(self, img_features, meta_features):
        ...
        return preds_r, preds_p, d_attns[-1], f_attns[-1], p_attns[-1]
    ...
    def create_masks(self, input, targets):
        progs_mask = targets['progs_mask']
        prognosis_mask = targets['prognosis_mask']
        label_len = input['label_len']

        # Combine 2 masks
        progs_mask = progs_mask.unsqueeze(dim=-1)
        prognosis_mask = prognosis_mask.unsqueeze(dim=-1)
        mask = torch.cat((progs_mask, prognosis_mask), dim=-1)

        progs_mask_1hot = progs_mask.repeat(1, 1, self.cfg.n_pr_classes)
        prognosis_mask_1hot = prognosis_mask.repeat(1, 1, self.cfg.n_pn_classes)
        mask_onehot = torch.cat((progs_mask_1hot, prognosis_mask_1hot), dim=-1)

        return mask, mask_onehot, progs_mask, prognosis_mask, label_len
    ...
    def fit(self, input, target, batch_i, n_iters, epoch_i, stage="train"):
        preds_r, preds_p = self.forward(input, batch_i, target)

        outputs = {}

        # Bound parameters
        self.apply_constraints_softmax_all(s=1.0)
        ...
        self.log_tau(batch_i, n_iters, epoch_i, stage)
        ...
        for grading in self.gradings:
            ...
            for t in range(0, self.seq_len):
                ...
                if t > 0:
                    head_index = self.output_map_p[f'{grading}_{t - 1}'][1]
                    ...
                    head_index = self.output_map_r[f'{grading}_{t}'][1]
                    pn_targets = target[f'prognosis_{grading}']
                    pn_masks = target[f'prognosis_mask_{grading}']
                    pn_logits_mask = preds_r[head_index][pn_masks[:, t], :]
                    pn_target_mask = pn_targets[pn_masks[:, t], t].to(self.device)
