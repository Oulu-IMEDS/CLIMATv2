...
def make_network(name, pretrained='imagenet', n_cls=1000, input_3x3=False):
    if name == 'senet154':
        ...
        net = CustomResnet(n_classes=n_cls, in_channels=3, n_features=64, drop_rate=0.2)
    else:
...
class CustomResnet(nn.Module):
    def __init__(self, in_channels=3, n_features=64, drop_rate=0.2, n_classes=10):
        super().__init__()
        c = [n_features, 2 * n_features, 4 * n_features, 6 * n_features, 8 * n_features]
        self.layer0 = nn.Sequential(nn.Conv2d(in_channels, n_features, kernel_size=3, stride=1, padding=1, bias=False),
                                    nn.Conv2d(n_features, n_features, kernel_size=3, stride=1, padding=1, bias=False))

        self.layer1 = nn.Sequential(ResBlock(c[0], c[1], 2),
                                    ResBlock(c[1], c[1], 1))

        self.layer2 = nn.Sequential(ResBlock(c[1], c[2], 2),
                                    ResBlock(c[2], c[2], 1))

        self.layer3 = nn.Sequential(ResBlock(c[2], c[3], 2),
                                    ResBlock(c[3], c[3], 1))

        self.layer4 = nn.Sequential(ResBlock(c[3], c[4], 2),
                                    ResBlock(c[4], c[4], 1))
