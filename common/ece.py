...
def process_input(input, labels):
    if not isinstance(input, torch.Tensor):
        if isinstance(input, np.ndarray):
            input = torch.tensor(input)
        elif isinstance(input, list) or isinstance(input, tuple):
            if len(input[0].shape) == 2:
                input = torch.tensor(np.concatenate(input, axis=0))
            elif len(input[0].shape) == 1:
...
class ECELoss(nn.Module):
    ...
    '''
    def __init__(self, n_bins=15, normalized=True):
    ...
    def forward(self, input, labels, return_tensor=True):
        input, labels = process_input(input, labels)

        if input.shape[0] == 0 or labels.shape[0] == 0:
            return None
...
class ClasswiseECELoss(nn.Module):
    ...
    def forward(self, input, labels, return_tensor=True):
        ...
        for i in range(num_classes):
            ...
            class_sce = torch.zeros(1, device=input.device)
